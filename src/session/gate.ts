 ...
 * tmux -e 注入 PI_SUBAGENT=1），与主会话侧的其余接线（index.ts、launch/、tools/）
 * 分属两个执行上下文——session/ 目录专门表达这个边界。
 */
import { writeFileSync } from "node:fs";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { isWatchdogAvailable } from "../completion/profile";
import { ENV_SUB_DONE, ENV_SUB_EXIT_FILE, ENV_SUBAGENT, ENV_WATCHDOG } from "../core/env";
 ...
 *
 * 自检（仅交互式路径，注入了 PI_WATCHDOG（ENV_WATCHDOG）时注册）：放到 session_start——此时各扩展
...
export function setupSelfCheck(pi: ExtensionAPI): boolean {
	if (process.env[ENV_SUBAGENT] !== "1") return false;

	if (process.env[ENV_WATCHDOG]) {
		onEvent(
			pi,
			"session_start",
			{
			...
			async (_event: unknown, ctx: { ui: { notify(text: string, level: string): void } }) => {
				...
				if (process.env[ENV_SUB_EXIT_FILE]) {
					try {
						writeFileSync(process.env[ENV_SUB_EXIT_FILE], "97\n");
					} catch {
						/* ignore */
					}
				}
				if (process.env[ENV_SUB_DONE]) {
					// detached：session_start 阻塞无意义，发完即走；wait-for -S 立即返回，
					// 信号被 server 记住，晚到的等待方也能拿到
					pi.exec("sh", ["-c", `TMUX= tmux -L ${SOCKET} wait-for -S ${shQuote(process.env[ENV_SUB_DONE])}`]).catch(
						() => {},
					);
				}
			},
